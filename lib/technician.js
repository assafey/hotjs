...
function find(region, date, time, callback) {
    ...
    if (!region) {
        ...
        technicians.find({region:region, schedule:{$elemMatch:{date:{$eq:date},time:{$ne:time}}}}, {tech_id:'', first_name:'', last_name:''})
        ...
        getById(techId, function(err, res) {
            if (!err && res) {
                ...
                if (!res.schedule) {
                    ...
                    technicians.update({tech_id:res.tech_id}, res, function(err, res) {
                        handler.call(callback, err, res);
                    });
                }
            } else {
